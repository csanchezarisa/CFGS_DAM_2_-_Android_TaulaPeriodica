...
public class ActivityGameEncertarSimbol extends AppCompatActivity {
    ...
    private void generaSimbol() {
        ...
        simbolElementActual = elements[posicioLlistat].getSimbol();

        TextView textView = (TextView) findViewById(R.id.txtSimbolElement);
        textView.setText(simbolElementActual);
    }

    // Comprova si el l'element introduit dins del dialog és el mateix que el que s'està mostrant
    private void comprovarResultat() {
        EditText input = (EditText) findViewById(R.id.edtNomElement);

        if (input.getText().toString().length() > 0) {

            if (input.getText().toString().equalsIgnoreCase(nomElementActual)) {
                partidesGuanyades ++;
                partidesJugades ++;
                puntuacio ++;
                setPuntuacions();
    ...
    private void snackbarDialogBuit() {
        View parentLayout = findViewById(android.R.id.content);
        Snackbar snack = Snackbar.make(parentLayout, Html.fromHtml("<font color=\"black\">INTRODUEIX UN NOM!</font>"), Snackbar.LENGTH_LONG);
    ...
    private void alertError() {
        ...
        alerta.setMessage("T'has equivocat, el nom de l'element " + simbolElementActual + " és " + nomElementActual);
